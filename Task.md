# Список завдань (Task.md) - Nova Poshta AI Bot v0.24.12
...
- [x] **Крок 30: Виправлення хибного розпізнавання банківської картки при створенні накладної (захист від склеювання розрізнених цифр повідомлення) (v0.24.11)**
  - [x] Виявити причину хибного спрацьовування: конкатенація всіх цифр тексту (`filter(str.isdigit, text)`), через що телефон + відділення + наложка давали 16 цифр
  - [x] Реалізувати `extract_standalone_bank_card` у `src/bot/handlers.py` з перевіркою суцільного блоку 16-19 цифр (`CARD_BLOCK_REGEX`) та виключенням реквізитів отримувача/телефонів
  - [x] Додати автоматичне скидання стану `USER_CARD_WAITING` при надсиланні накладної або тексту без картки
  - [x] Додати підтримку збереження картки виплати при її явній вказівці всередині тексту накладної
  - [x] Написати юніт-тести в `tests/test_card_session_resume.py` на перевірку форматування та відхилення склеєних цифр посилки
  - [x] Успішно виконати всі 148 тестів проекту паралельно (`python -m pytest -n auto`)
  - [x] Оновити `Walkthrough.md`, `Task.md`, `plan.md` та `README.md`

- [x] **Крок 31: Псевдоніми облікових записів відправників, 2-рядкові кнопки лімітів для мобільних пристроїв та автодисамбігуація однакових ПІБ (v0.24.12)**
  - [x] Розділити розмітку кнопок користувачів у `get_users_management_keyboard` на 2 окремі рядки (Рядок 1: ім'я/вибір профілю, Рядок 2: виділена повноширинна кнопка залишку ліміту `💰 Залишок: {сума} грн`), усунувши обрізання тексту трикрапкою на мобільних пристроях.
  - [x] Додати поле `alias: Optional[str] = None` до моделі `SenderProfile` у `src/storage.py`.
  - [x] Реалізувати методи `rename_sender_profile` та `get_sender_profile` у `UserSettingsManager` з підтримкою збереження та скидання псевдоніма.
  - [x] Додати інтерактивну дію меню `[ ✏️ Змінити псевдонім ]`, клавіатуру вибору `get_rename_profile_selection_keyboard` та діалогове введення псевдоніма через `USER_RENAME_PROFILE_WAITING`.
  - [x] Додати команди `/set_alias [Назва]`, `/set_name`, `/rename_user` та `/reset_alias`.
  - [x] Реалізувати функцію `format_profile_display_name` для інтелектуального скорочення повних імен до `Прізвище І. П.` та автоматичного додавання 4 останніх цифр телефону `(..XXXX)` при наявності акаунтів з однаковими ПІБ.
  - [x] Оновити тексти дашборду `_render_users_dashboard` для виведення псевдоніма поруч з офіційним ПІБ і телефоном.
  - [x] Написати нові юніт-тести в `tests/test_client_card_and_ui.py` та `tests/test_multi_user_profiles.py`.
  - [x] Успішно виконати всі 151 тест проекту паралельно (`python -m pytest -n auto`).
  - [x] Оновити `Walkthrough.md`, `Task.md`, `plan.md` та `README.md`.
