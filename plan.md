...
## Основні компоненти (Виконано у v0.4.0)
1. [x] **AI Entity Extractor & Conversational Chat (`src/ai/`)**:
   - Підтримка 3-х AI провайдерів через `openai` / `httpx` SDK.
...
5. [x] **Тестування на реальних даних**:
   - Успішно протестовано генерацію ТТН та її видалення через Nova Poshta API.
