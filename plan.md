...
## Основні компоненти (Виконано у v0.1.3)
...
2. [x] **Nova Poshta API Client (`src/nova_poshta/`)**:
   ...
   - `getWarehouses` — пошук конкретного відділення або поштомату за номером та Ref міста (підтримка `FindByString` для швидкого пошуку поштоматів).
   - `Counterparty/save` & `ContactPerson/save` — створення отримувача.
   - `InternetDocument/save` — створення ТТН (з можливістю вибору платника: Отримувач / Відправник, авто-заповнення `OptionsSeat`).
...
5. [x] **Тестування на реальних даних**:
   - Успішно протестовано генерацію ТТН № `20451506103956` через Nova Poshta API від відправника з м. Івано-Франківськ до отримувача в м. Київ (Поштомат №26584).
