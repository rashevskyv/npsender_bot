...
- **🏡 Intelligent Courier Address Delivery Support ("🏡 Адресна доставка кур'єром")**: Seamlessly detects and creates courier door delivery waybills (`WarehouseDoors`)! When recipient info specifies an address, the bot uses multi-permutation street search (e.g. mapping *"вул. Віри Гордієнко"* -> *"вул. Гордієнко Віри"*) with prefix stripping and similarity ranking, validates street existence via `Address/getStreet`, registers the recipient address via `Address/save`, and creates the door delivery waybill.
