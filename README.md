...
- **📋 AI-Powered ScanSheet Registers, Natural Language Editing & Code128 Barcodes (`/registers`)**: Automatically builds, modifies, and manages Nova Poshta ScanSheet registers using LLM intelligence over user waybill drafts!
  ...
  - **Automatic Corrupt Register Filtering & Context Recovery**: If bot restarts or memory context is cleared, it recovers the genuine active register from storage while strictly filtering out corrupt entries with missing numbers or refs, ensuring exact item indexing across multi-waybill registers.
