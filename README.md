...
- **📋 ScanSheet Registers & Code128 Barcode Generation (`/registers`)**: Create Nova Poshta ScanSheet registers from waybills filtered by date, time of day, or cargo description via natural language (e.g. *"створи реєстр з усіх накладних з описом сувенір"* or *"надай всі накладні за сьогодні"*). Displays only active un-shipped registers created within the last 2 days (`days_back=2`), automatically filtering out older or already shipped registers, and sends high-resolution Code128 PNG barcode images in Telegram for instant scanning at Nova Poshta branches!
...
- **🔐 Strict Multi-Tenant & Per-User Settings Isolation**: Each Telegram user has their own isolated profile in `user_settings.json`. New unconfigured users are protected by an Onboarding Guard (`ensure_user_configured`) and must bind their own API key (`/set_np_key`), ensuring zero data leakage between different Telegram users. Automatically fetches sender credentials from Nova Poshta API upon key entry!
...
- **📤 Outgoing Active Shipments (`/outgoing` / `📤 Вихідні (що їдуть)`)**: Lists all active outgoing packages sent by the user (matched by sender phone/name) that have not yet been picked up by the recipient.
- **📥 Incoming Active Shipments (`/incoming` / `📥 Вхідні (що їдуть)`)**: Lists all incoming packages traveling to the user (matched by recipient phone/name) with smart relative delivery date formatting (*"Сьогодні о 18:00"*, *"Завтра о 15:30"*, *"Післязавтра"*, or *"DD.MM.YYYY"*). Filtered to show only uncollected shipments.
